...
![Diagram](./divide-conquer-count.png)

---

## 1. The problem

![One count query pins one core while the rest sit idle](./diagrams/s1-problem.png)
...
![Cut the _id number line into K disjoint tiles](./diagrams/s2-partition.png)
...
![Fan-out: one sub-query per range, each on its own core](./diagrams/s3-fanout.png)

![Converge: sum the K partial counts into one exact total](./diagrams/s4-converge.png)
...
![Seek the security interval, then bound the _id slice inside it](./diagrams/s5-index.png)
...
![Uniform cuts on time-clustered ids produce skewed buckets](./diagrams/s8-skew.png)
